...
It is important to note in the proof of Theorem 13.10 that the ``simplifying assumptions'' are actually a load-bearing and important part of the actual reduction algorithm. For example, the first assumption is ``that $\adv'$ makes any given query to $H$ only once.'' The justification for such an assumption is that if we had an attacker $\bdv'$ that was allowed to query $H$ multiple times with the same input, then we could build an attacker $\adv'$ that didn't by calling $\bdv'$ as a subroutine and forwarding all messages to the environment and forwarding all responses back to $\bdv'$, except that we don't forward redundant queries to $H$ and instead keep a cache of all previous queries and respond directly using the cache when possible. Thus, for any ``general'' attacker $\bdv'$, there exists an attacker $\adv'$ with equal advantage that never makes redundant calls to $H$. Assumptions such as these are usually signaled by the preceeding phrase ``without loss of generality.'' Thus, the complete reduction in Theorem 13.10 should be thought of as composing the algorithm $\adv$ that they provide with various other simple wrapping reductions tied to the simplifying assumptions.

\begin{exercise}
Justify all of the simplifying assumptions made at the beginning of the proof of Theorem 13.10 in the above reading.
\end{exercise}
...
I would like to clarify the second-to-last step in the string of inequalities showing that $\pr{\mathsf{DLog}_{\adv', \mathcal{G}}(n) = 1} \geq \delta(n)^2 - \frac{1}{q}$; specifically, why $\sum_\omega \pr{\omega}\cdot (\delta_\omega)^2\geq \left(\sum_\omega \pr{\omega}\cdot \delta_\omega\right)^2.$ There is a clever trick involved (that you will not be expected to be able to come up with on your own as it comes from another field).

Define a function $f(t) := \sum_\omega \pr{\omega}\cdot (\delta_\omega - t)^2$ that takes in a real number $t\in\mathbb{R}$ and returns a non-negative real number (since it is a sum of products of non-negative numbers). Expanding the definition of $f(t)$, we get that
\begin{align*}
f(t) &= \sum_\omega \pr{\omega}\cdot (\delta_\omega - t)^2\\
&= \left(\sum_\omega \pr{\omega}\right)\cdot t^2 + \left(-2\sum_\omega\pr{\omega}\cdot\delta_\omega\right)\cdot t + \sum_\omega\pr{\omega}\cdot(\delta_\omega)^2\\
& = t^2 + \left(-2\sum_\omega\pr{\omega}\cdot\delta_\omega\right)\cdot t + \sum_\omega\pr{\omega}\cdot(\delta_\omega)^2,
\end{align*}
which is a quadratic function of $t$. Recal the quadratic formula, that $$at^2 + bt + c = 0\Rightarrow t = \frac{-b\pm\sqrt{b^2 - 4ac}}{2a}.$$ If a quadratic function such as $f(t)$ is always non-negative, then this means it has either $0$ or $1$ solutions to $at^2 + bt + c = 0$, and this is equivalent to saying the statement that $b^2 - 4ac\leq 0$ (as otherwise there would be $2$ solutions). For our quadratic function $f(t)$, this implies that
\begin{align*}
\left(-2\sum_\omega\pr{\omega}\cdot\delta_\omega\right)^2 - 4\sum_\omega\pr{\omega}\cdot(\delta_\omega)^2&\leq 0,\text{ and so}\\
4\left(\sum_\omega\pr{\omega}\delta_\omega\right)^2&\leq 4\sum_\omega\pr{\omega}(\delta_\omega)^2;\text{ equivalently,}\\
\left(\sum_\omega\pr{\omega}\delta_\omega\right)^2&\leq \sum_\omega\pr{\omega}(\delta_\omega)^2.
\end{align*}
Note that we used the fact that probabilities are non-negative and add up to $1$.\\

Now let's work towards another proof of security for Schnorr signatures! In 2018, Fuchsbauer, Kiltz, and Loss formalized the Algebraic Group Model (AGM). In the AGM, adversaries have the restriction that every time an ``algebraic adversary'' outputs a group element it must also output a list of exponents that show how to compute the group element as a combination of the group elements that have been given to the adversary as inputs (or public parameters). For example, recall the Computational Diffie-Hellman (CDH) assumption that it is hard to compute $g^{xy}$ given $X = g^x$ and $Y = g^y$ as input. An algebraic adversary (i.e., adversary in the AGM) that plays the CDH attack game is given $X$ and $Y$ (as well as the generator $g$) and it must output not only a group element $Z$, but also a list $(a, b, c)$ such that $Z = g^aX^bY^c$.\\
...
\item Show that if $(P, V)$ provides special soundness, then so does $(P^k, V^k)$.
